...
for (const locale of context.window.Yog1Locales) {
    ...
    const filename = 'translations/' + locale.id + '.js';
...
assert.strictEqual(precacheList.includes('./translations/'), false,
...
for (const locale of i18n.localeOptions.filter(function (item) { return item.id !== 'en'; })) {
    assert(serviceWorker.includes("'./translations/" + locale.id + ".js'"),
        locale.id + ' is listed as a lazy, runtime-cached locale bundle');
}
assert(html.indexOf('<script src="locale-loader.js"></script>') <
    html.indexOf('<script src="i18n.js"></script>'),
'the selected locale bundle loads before the localization runtime');
assert(fs.statSync('i18n.js').size < 30000,
...
vm.runInNewContext(fs.readFileSync('locales.js', 'utf8'), loaderContext,
    { filename: 'locales.js' });
vm.runInNewContext(fs.readFileSync('locale-loader.js', 'utf8'), loaderContext,
    { filename: 'locale-loader.js' });
assert(writtenLocaleScript.includes('translations/zh-Hant.js'),
...
const lazyContext = {
    ...
    window: { location: { search: '', href: 'https://example.test/yog1.htm' } }
};
...
vm.runInNewContext(fs.readFileSync('locales.js', 'utf8'), lazyContext,
    { filename: 'locales.js' });
vm.runInNewContext(fs.readFileSync('i18n.js', 'utf8'), lazyContext,
    { filename: 'i18n.js' });
